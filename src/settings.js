// src/settings.js
// Constants + buildSettings

/* ===== KV Keys (export supaya bisa dipakai modul lain) ===== */
...
function bool(v, d=false){ return v==null ? d : ['1','true','yes','on'].includes(String(v).toLowerCase()); }
...
export function buildSettings(env){
  ...
  if (env.WILDCARD_MAP_JSON) {
    try { const j = JSON.parse(env.WILDCARD_MAP_JSON); if (j && typeof j === 'object') WILDCARD_MAP = j; } catch {}
  }
...
export default buildSettings;    META_TIMEOUT_MS: num(env.META_TIMEOUT_MS, 1500),    // timeout tiap IP
